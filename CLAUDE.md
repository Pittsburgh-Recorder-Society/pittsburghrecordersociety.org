...
**Astro static site** (SSG mode) with Tailwind CSS, hosted on Netlify.
...
Three custom color scales defined in `tailwind.config.mjs`:
...
**Authentication**: The `config.yml` backend is set to `git-gateway` with `local_backend: true` for local dev. For production, DecapBridge URLs need to be configured (see TODO in `config.yml`). Date fields use `z.coerce.date()` in `config.ts` to handle both `YYYY-MM-DD` (hand-written) and ISO timestamp (CMS-generated) formats.
