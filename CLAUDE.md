...
**Dependency pin (do not "fix" this)**: `decap-server` is pinned to the exact version `3.11.0` in
`devDependencies`, deliberately not a `^` range. Version `3.11.1` was published from a pnpm monorepo with its
dependency specifiers left as the literal string `catalog:` (a pnpm workspace catalog feature). npm has no
`catalog:` protocol handler, so any `npm install` that resolves to 3.11.1 aborts with
`EUNSUPPORTEDPROTOCOL: Unsupported URL Type "catalog:"` while building the ideal tree, and the lockfile cannot
be regenerated. Because `npm ci` only replays the committed lockfile, CI can stay green while local installs
are broken, which makes this failure easy to misattribute to Astro or to the site code. Widen the pin only
after confirming a newer `decap-server` release lists real semver ranges:
`npm view decap-server@<version> dependencies`.
